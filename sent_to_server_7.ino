void send_to_server_7() {
  ...
  url += "&un=1";
  url += "&n1=";
  url += String(sent_nr_7);  //sensor value
...
URL_withPacket += "Connection: close\r\n\r\n";  
  */
  ...
  if (connect_ESP()) {                                                                //this calls 'connect_ESP()' and expects a '1' back if successful
    ...
    if (read_until_ESP(keyword_n7, sizeof(keyword_n7), 5000, 0)) {                    //same as before - first d1
      if (read_until_ESP(keyword_doublehash, sizeof(keyword_doublehash), 5000, 1)) {  //now ## and mode=1
        ...
        //delay(10);
        Serial.println("FOUND DATA & DISCONNECTED SENZOR 7");
        serial_dump_ESP();  //now we can clear out the buffer and read whatever is still there
        //let's see how the data looks

        Serial.print("SENT_NUMBER_7 = ");  //print out LED data and convert to integer
        sent_nr_7_feedback = 0;
        for (int i = 1; i <= d10_from_ESP[0]; i++) {
          //Serial.print(d1_from_ESP[i]);
          sent_nr_7_feedback = sent_nr_7_feedback + ((d10_from_ESP[i] - 48) * multiplier[d10_from_ESP[0] - i]);
        }
        Serial.println(sent_nr_7_feedback);
        Serial.println("");
        //that's it!!
      }  //##
    }    //d2
